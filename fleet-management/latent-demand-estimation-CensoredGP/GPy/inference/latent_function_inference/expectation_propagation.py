...
class EPCensored(EPBase, ExactGaussianInference):
    def inference(self, kern, X, likelihood, Y, censoring, mean_function=None, Y_metadata=None, precision=None, K=None):
        ...
        if self.ep_mode=="nested" and not self.loading:
            #Force EP at each step of the optimization
            self._ep_approximation = None
            post_params, ga_approx, cav_params, log_Z_tilde = self._ep_approximation = self.expectation_propagation(mean_prior, K, Y, censoring, likelihood, Y_metadata)
        elif self.ep_mode=="alternated" or self.loading:
            if getattr(self, '_ep_approximation', None) is None:
                #if we don't yet have the results of runnign EP, run EP and store the computed factors in self._ep_approximation
                post_params, ga_approx, cav_params, log_Z_tilde = self._ep_approximation = self.expectation_propagation(mean_prior, K, Y, censoring, likelihood, Y_metadata)
    ...
    def expectation_propagation(self, mean_prior, K, Y, censoring, likelihood, Y_metadata):
        ...
        iterations = 0
        while not stop and (iterations < self.max_iters):
            self._local_updates(num_data, cav_params, post_params, marg_moments, ga_approx, likelihood, Y, censoring, Y_metadata)
    ...
    def _local_updates(self, num_data, cav_params, post_params, marg_moments, ga_approx, likelihood, Y, censoring, Y_metadata, update_order=None):
            if update_order is None:
                update_order = np.random.permutation(num_data)
            for i in update_order:
                #Cavity distribution parameters
                ...
                #Marginal moments
                marg_moments.Z_hat[i], marg_moments.mu_hat[i], marg_moments.sigma2_hat[i] = likelihood.moments_match_ep(Y[i], cav_params.tau[i], cav_params.v[i], censoring[i], Y_metadata_i=Y_metadata_i)
